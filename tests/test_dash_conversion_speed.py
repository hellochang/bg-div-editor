...
app.layout = dbc.Container([
    ...
    dbc.Label('lst dict'),
    dash_table.DataTable(id='filter-lst'),
    
    dbc.Label('fsym id-data-table'),
    dash_table.DataTable(id='filter-pd'),  



    dash_table.DataTable(id='fsym-id-data-table'),
    dash_table.DataTable(id='filter_pd_json-data-table'),

    dash_table.DataTable(id='filter_json-data-table'),
...
def load_data_to_dash(update_date):
    ...
    return new_data.to_dict('records'), dropdown, new_data['fsym_id'].unique()[0],\
        cols, cols
